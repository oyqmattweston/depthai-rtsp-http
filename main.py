...
parser.add_argument("-cam_ip", "--camera_ip_address", required= True, type=str, help="IP address of the device")
...
parser.add_argument("-qa",  "--quality",   default=60,         type=int,   help="Video quality, from 1 to 100")
...
parser.add_argument("-fps",  "--frames_per_second",default=30,        type=int,  help="Frames per second of video, default is 30")
parser.add_argument("-path", "--rtsp_path", default="preview", type=str, help="Path of the RTSP stream, default is /preview")
...
CAMERA_IP_ADDRESS   = args["camera_ip_address"]
...
if __name__ == "__main__":
    ...
    videnc.setDefaultProfilePreset(FPS, dai.VideoEncoderProperties.Profile.H264_HIGH)
    videnc.setKeyframeFrequency(FPS * 4)
    videnc.setQuality(QUALITY)
    videnc.setRateControlMode(dai.VideoEncoderProperties.RateControlMode.VBR)
    ...
    device_info = dai.DeviceInfo(CAMERA_IP_ADDRESS)
    ...
    #     ctrl.setCaptureStill(True)PyGObject==3.38.0
    ...
    command = [
        ...
        "-vcodec", "copy",
        "-v", "error",
        f"rtsp://{credential}{RTSP_HOST}:{RTSP_PORT}/{RTSP_PATH}"
    ]
    ...
    with dai.Device(pipeline, device_info) as device:
        ...
        print(f"Setup finished, RTSP stream available under \"rtsp://{RTSP_HOST}:{RTSP_PORT}/{RTSP_PATH}\"")
